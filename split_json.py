...
def write_json(js, json_file):
    with open(json_file, 'wb') as f:
        f.write(json.dumps(js, indent=4, sort_keys=True).encode('utf_8'))
...
if __name__ == '__main__':
    ...
    js_ori = json.load(open(args.json, 'rb'))
    js = js_ori["fileInfo"]
    ...
    for i, new_j in enumerate(new_js):
        ...
        write_json({"fileType":js_ori["fileType"],"fileInfo":new_j},
                   os.path.join(args.datadir, file_name))