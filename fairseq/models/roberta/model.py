...
class RobertaModel(FairseqEncoderModel):
    ...
    def build_model(cls, args, task):
        ...
        if hasattr(task, "target_dictionary") and len(task.target_dictionary) != len(task.source_dictionary):
            encoder = RobertaEncoder(args, task.source_dictionary,task.trg_dictionary)
        else:
...
class RobertaEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, trg_dictionary=None):
        ...
        if trg_dictionary is not None:
            self.lm_head = self.build_lm_head(
                embed_dim=args.encoder_embed_dim,
                output_dim=len(trg_dictionary),
    ...
    def reorder_encoder_out(self, encoder_out, new_order):
        ...
        new_out = encoder_out[0].index_select(
            0, new_order
        )
        encoder_out_new = (new_out, encoder_out[1])
