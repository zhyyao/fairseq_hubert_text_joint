...
@dataclass
class ElHubertConfig(FairseqDataclass):
...
@register_model("el_hubert", dataclass=ElHubertConfig)
class ElHubertModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: ElHubertConfig,
        ...
        if self.separate_label_embeds:
            if self.separate_layer_targets or not self.untie_final_proj:
                self.final_proj = torch.nn.Sequential(*[nn.Linear(
                    cfg.encoder_embed_dim, final_dim)
        ...
        if any([d is None for d in dictionaries]):
            ...
            self.label_embs_concat = nn.Parameter(
                torch.FloatTensor(layer_dim, embed_dim, final_dim)
            )
    ...
    @classmethod
    def build_model(cls, cfg: ElHubertConfig, task: HubertPretrainingTask):
        """Build a new model instance."""

        model = ElHubertModel(cfg, task.cfg, task.dictionaries)
        return model
    ...
    def forward(
        ...
        if output_layer is None:
            output_layer = self.encoder_layers
        ...
        assert len(layer_results) == self.encoder_layers
        layer_results = [layer_x.transpose(0, 1) for i, (layer_x, _) in enumerate(layer_results) if (i+1) in self.predict_layers]
