...
@register_model("ce_hubert", dataclass=HubertConfig)
class HubertModel(BaseFairseqModel):
    ...
    def forward(
        ...
        if not self.skip_masked:
            ...
            proj_x_m = proj_x[masked_indices]
            logit_m_list = [proj_x_m]
            target_m = [target_list[0][masked_indices].long()]
        else:
            logit_m_list = [None for _ in target_list]
            target_m = [None]
        if not self.skip_nomask:
            nomask_indices = torch.logical_and(~padding_mask, ~mask_indices)
            proj_x_u = proj_x[nomask_indices]

            logit_u_list = [proj_x_u]
            target_u = [target_list[0][nomask_indices].long()]
        else:
        ...
        result["target_m"] = target_m
        result["target_u"] = target_u
        result["padding_mask"] = padding_mask
        result["features_pen"] = features_pen
        result['cnn_time'] = cnn_time
        result["encoder_time"] = encoder_time
        result["logit_time"] = logit_time
        return result
    ...
    def get_targets(self, net_output, is_masked=True):
        if is_masked:
            targets_list = net_output['target_m']
        else:
            targets_list = net_output['target_u']
        return targets_list
