...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        if args.encoder_layers == 12:
            self.apply(init_bert_params)

    def forward(self, x, padding_mask=None, streaming_mask=None, layer=None, pos_bias=None):
        x, layer_results = self.extract_features(x, padding_mask, streaming_mask, layer, pos_bias)
    ...
    def extract_features(self, x, padding_mask=None, streaming_mask=None, tgt_layer=None, pos_bias=None):
        if padding_mask is not None:
        ...
        for i, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability > self.layerdrop):
                x, z, pos_bias, lr = layer(x, self_attn_padding_mask=padding_mask, need_weights=False, self_attn_mask=streaming_mask, pos_bias=pos_bias)
                if tgt_layer is not None:
                    layer_results.append((x, z, pos_bias, lr))
            else:
                layer_results.append((x, None, pos_bias, lr))
            if i == tgt_layer:
                r = x
                break
...
class TransformerSentenceEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.layer_norm_first:
            self.self_attn_layer_norm_sandwich = LayerNorm(self.embedding_dim,elementwise_affine=True)
        ...
        if self.layer_norm_first:
            self.final_layer_norm_sandwich = LayerNorm(self.embedding_dim,elementwise_affine=True)
    ...
    def forward(
        ...
        return x, attn, pos_bias, layer_result


class TransformerCrossAttnSentenceEncoderLayer(nn.Module):
    """
    Implements a Transformer Encoder Layer used in BERT/XLM style pre-trained
    models.
    """
    ...
    def __init__(
        ...
        # layer norm associated with the self attention layer
        self.self_attn_layer_norm = LayerNorm(self.embedding_dim,)
        self.attn_layer_norm = LayerNorm(self.embedding_dim,)
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            if encoder_out != None:
                ...
                x, attn, pos_bias = self.encoder_attn(
                    query=x,
                    key=encoder_out,
                    value=encoder_out,
