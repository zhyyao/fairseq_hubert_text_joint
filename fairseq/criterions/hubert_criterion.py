...
class HubertCriterion(FairseqCriterion):
    ...
    def get_loss(self, model, sample, net_output, reduce=True, log_pred=False):
        ...
        if self.pred_masked_weight > 0:
            if isinstance(model, ElHubertModel):
        ...
        logging_output = {
            ...
            "loss_hubert": loss.item() if reduce else loss,
            "ntokens": sample_size,
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        # start = time.time()
        loss_mlm = None
        if "pair_loss" in net_output.keys() and net_output["pair_loss"] is not None:
            loss_paired = F.mse_loss(net_output["pair_loss"][0], net_output["pair_loss"][1],reduction='sum')


        #if "phf_out" in net_output.keys() and net_output["phf_out"] is not None:

        #    probs_text =  model.get_normalized_probs(
        #        net_output["phf_out"] ,log_probs=True
        #    ).contiguous()
        #    # (B, T, C) from the encoder
        #    ph_target = sample["net_input"]["phf_input"].long()
        #    # start = time.time()
        ...
        #     pad_mask = (sample["target_list"][2] != self.pad_idx) & (
        #         sample["target_list"][2] != self.eos_idx
        #     )
        #     targets_flat = sample["target_list"][2].masked_select(pad_mask)
        #     if "target_lengths_list" in sample:
        #         target_lengths = sample["target_lengths_list"][2]
        #     else:
        ...
        if loss_mlm is not None:
            loss += loss_mlm * 0.3
        if "pair_loss" in net_output.keys() and net_output["pair_loss"] is not None:
            loss_paired = F.mse_loss(net_output["pair_loss"][0], net_output["pair_loss"][1],reduction='mean')
            loss+= loss_paired * 0.3


        # print("speech only:", loss)
        # end = time.time()
        # loss_time = end-start
            logging_output["loss_paired"] = loss_paired
        # logging_output["time_speech_only_forward"] = forw_time
        # logging_output["time_speech_only_loss"] = loss_time
        # logging_output["time_speech_only_total"] = forw_time + loss_time
        return loss, sample_size, logging_output
