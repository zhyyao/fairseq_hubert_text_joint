...
class MaskTokensDataset(BaseWrapperDataset):
    ...
    def __init__(
        ...
        self.s_idx = vocab.index("<s>")
        self.e_idx = vocab.index("</s>")
        ...
        #if "<sil>" in vocab.symbols:
        #    self.sil = vocab.index("<sil>")
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            choice_weight=None
            if hasattr(self, "sil"):
                sil_idc = ((item==self.sil).int() + (item == self.e_idx).int() + (item==self.s_idx).int()).bool()
                choice_weight = (sil_idc==False).int()
                can_mask = torch.sum(choice_weight)
                idx = torch.Tensor(range(0,sz))
                tmp2 = idx * choice_weight
                nonzero_ind = torch.argmax(tmp2, keepdim=True)
                choice_weight = choice_weight / can_mask
                choice_weight[nonzero_ind] = 1- torch.sum(choice_weight[:nonzero_ind])

            if choice_weight == None:
                can_mask = sz
            ...
            num_mask = int(
                ...
                self.mask_prob * can_mask / float(self.mask_multiple_length)
                + np.random.rand()
            )

            # multiple masking as described in the vq-wav2vec paper (https://arxiv.org/abs/1910.05453)
            mask_idc = np.random.choice(sz, num_mask, replace=False, p=choice_weight)
