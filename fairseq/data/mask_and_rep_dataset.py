...
class RepProcess(object):
    def __init__(self,dic,path):
        f = open(path)
        map = { l.split()[0]: int(l.split()[1])//2 for l in f.readlines() }
        f.close()
        self.rep_len = [0 for i in range(len(dic))]
        self.dic = dic
        for s in dic.symbols :
            if s in map.keys():
                self.rep_len[dic.index(s)] = map[s]
            else:
                self.rep_len[dic.index(s)] = 1
        # import pdb
        # pdb.set_trace()
    
    def __call__(self, item, mask, do_mask=True):
        rp_len = [self.rep_len[i] for  i in item]
        sz = sum(rp_len)
        new_item = np.full(sz, 0)
        new_mask = None
        if do_mask :
            new_mask = np.full(sz, False)
        base = 0
        for ind, i in enumerate(rp_len):
            new_item[base: base+i] = item[ind]
            if do_mask :
                new_mask[base: base+i] = mask[ind]
            base += i
        return new_item,new_mask, sz
        



class MaskAndRepDataset(BaseWrapperDataset):
    """
    ...
    def __init__(
        ...
        if rep_proc is not None:
            self.rep_proc = RepProcess(vocab, rep_proc)
            self.new_size = []
            o = open("phf_size.tsv")
            # import tqdm
            # for i in tqdm.tqdm(self.dataset):
            #     _,_,sz = self.rep_proc(i, None, False)
            #     o.write(str(sz)+"\n")
            # o.close
            # import sys
            # sys.exit(1)
            for i in o.readlines():
                self.new_size.append(int(i)//2)
            o.close()
        else:
            self.new_size = self.dataset.sizes
    ...
    def __getitem_cached__(self, seed: int, epoch: int, index: int):
        with data_utils.numpy_seed(self.seed, self.epoch, index):
            ...
            if self.rep_proc is not None:
                item, mask, sz = self.rep_proc(item,mask)
